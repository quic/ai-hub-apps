...
.EXAMPLE
    # 1. Export the model
    python -m qai_hub_models.models.llama_v3_2_3b_chat_quantized.export --device "Snapdragon X Elite CRD" --skip-inferencing --skip-profiling --output-dir genie_bundle

    # 2. Download tokenizer to genie_bundle\tokenizer.json (see ..\README.md for download links).

    # 3. Run the model on this device.
    & .\RunLlm.ps1 -ModelName llama_v3_2_3b_chat_quantized -BundleRoot genie_bundle -RawPrompt "<|begin_of_text|><|start_header_id|>user<|end_header_id|>\n\nHow many dogs are there in space?<|eot_id|><|start_header_id|>assistant<|end_header_id|>"
...
param (
    [Parameter(Mandatory = $true,
               HelpMessage = "The name of the AI Hub model to run,`n  e.g., llama_v3_2_3b_chat_quantized.")]
    [string]$ModelName,
