...
if [ "$USE_DOCKER" -eq 0 ]; then
    ...
    echo "::step::Running semantic_segmentation_ubuntu_py natively"
    exec bash "$SCRIPT" "${APP_ARGS[@]}"
fi

if [ ! -f "$APP_DIR/Dockerfile" ]; then
    echo "::error::No Dockerfile found for semantic_segmentation_ubuntu_py. Re-run with --no-docker to run natively." >&2
    exit 1
fi
...
echo "::step::Running semantic_segmentation_ubuntu_py in Docker"
